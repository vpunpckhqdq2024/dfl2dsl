...
PYTHON_FRONTIERS_CHECKPOINT = "python_frontiers.json"
SAMPLES_CHECKPOINT = "samples.json"
N_TRAIN, N_TEST = "n_tasks_train", "n_tasks_test"
...
class ExperimentState:
    ...
    SAMPLES = "samples"
    FRONTIERS = "frontiers"
    PYTHON_FRONTIERS = "python_frontiers"
    ...
    def __init__(self, config):
        ...
        self.tasks, self.task_frontiers, self.task_frontiers_python = self.init_tasks_from_config(config)
        if self.metadata["no_lang"]:
            self.task_language = {
                TRAIN: {task.name: [] for task in self.task_frontiers[TRAIN]},
                TEST: {task.name: [] for task in self.task_frontiers[TEST]}}
            self.task_vocab = {TRAIN: [], TEST: []}
        else:
            self.task_language, self.task_vocab = self.init_task_language_from_config(config)
    ...
    def init_tasks_from_config(self, config):
        ...
        # TODO: Override types by argument.
        # TODO: Override DSL by argument.
        self.tasks = task_loader.load_tasks(config[METADATA][TYPE_OVERRIDE])

        if config[METADATA][N_TRAIN] is not None:
            self.tasks[TRAIN] = self.tasks[TRAIN][:config[METADATA][N_TRAIN]]
        if config[METADATA][N_TEST] is not None:
            self.tasks[TEST] = self.tasks[TEST][:config[METADATA][N_TEST]]

        self.task_frontiers = {
            split: {task: Frontier([], task=task) for task in self.tasks[split]}
            for split in self.tasks.keys()
        }
        self.task_frontiers_python = {
            split: {task: Frontier([], task=task) for task in self.tasks[split]}
            for split in self.tasks.keys()
        }

        return self.tasks, self.task_frontiers, self.task_frontiers_python
    ...
    def log_frontiers(self, verbosity=LOG_DEBUG, include_samples=False):
        ...
        for task_split in self.task_frontiers:
            ...
            num_solved_python = len(self.get_non_empty_python_frontiers_for_split(task_split))
            print(
                f"\t total_solved_tasks_python_{task_split} @ iteration {self.curr_iteration}: {num_solved_python} / {len(self.task_frontiers_python[task_split])}"
            )
    ...
    def maybe_resume_from_checkpoint(self):
        ...
        if self.metadata[INIT_FRONTIERS_FROM_CHECKPOINT]:
            ...
            if (
                ...
                # Load the python frontiers.
                frontiers_python_loaded = self.load_frontiers_from_checkpoint(
                    use_resume_checkpoint=use_resume_checkpoint, python=True
                )
    ...
    def checkpoint_python_frontiers(self):
        json_frontiers = {}
        json_frontiers["_summary"] = {
            "n_tasks_solved": {
                split: len(self.get_non_empty_python_frontiers_for_split(split))
                for split in self.task_frontiers_python
            }
        }

        json_frontiers.update({split: {} for split in self.task_frontiers_python})
        for split in self.task_frontiers_python:
            for task in self.task_frontiers_python[split]:
                frontier_json = self.task_frontiers_python[split][task].json()
                json_frontiers[split][task.name] = frontier_json

        checkpoint_directory = os.path.join(
            self.get_checkpoint_directory(), PYTHON_FRONTIERS_CHECKPOINT
        )
        with open(checkpoint_directory, "w") as f:
            json.dump(json_frontiers, f, indent=4)
        print(
            f"============Checkpointing python frontiers to {checkpoint_directory}==========="
        )

    def load_frontiers_from_checkpoint(self, use_resume_checkpoint=False, python=False):
        ...
        checkpoint_name = PYTHON_FRONTIERS_CHECKPOINT if python else FRONTIERS_CHECKPOINT
        frontiers_checkpoint = os.path.join(checkpoint_dir, checkpoint_name)
        ...
        for split in self.task_frontiers:
            for task in self.task_frontiers[split]:
                if task.name in json_frontiers[split]:
                    ...
                    if python:
                        ps = json_frontier["programs"]
                        json_frontier["programs"] = []
                        loaded_frontier = Frontier.from_json(
                            task, self.models[model_loaders.GRAMMAR], json_frontier
                        )
                        loaded_frontier.entries = [
                            FrontierEntry(
                                program=p["program"], logLikelihood=p["logLikelihood"], origin=p["origin"],
                                logPrior=0., tokens=[]) for p in ps
                        ]
                        self.task_frontiers_python[split][task] = self.task_frontiers_python[split][
                            task
        ...
        f"============Loaded previously checkpointed {'python ' if python else ''}frontiers from {frontiers_checkpoint}==========="
        return True
    ...
    def checkpoint_state(self, state_to_checkpoint):
        for state in state_to_checkpoint:
            ...
            elif state == self.PYTHON_FRONTIERS:
                self.checkpoint_python_frontiers()
    ...
    def get_non_empty_python_frontiers_for_split(self, task_split):
        """Returns the non empty frontiers within a split."""
        return [
            self.task_frontiers_python[task_split][task]
            for task in self.task_frontiers_python[task_split]
            if not self.task_frontiers_python[task_split][task].empty
        ]
    ...
    def convert_frontiers_beta_normal(self):
        """Converts frontiers to beta-normal form."""
        for task_split in self.task_frontiers:
            for frontier in self.task_frontiers[task_split].values():
