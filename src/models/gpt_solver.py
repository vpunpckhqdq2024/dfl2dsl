...
Queries GPT to solve tasks in a DSL.
    ...
    def infer_programs_for_tasks(
        ...
        for task_i, task_id in enumerate(task_batch_ids):
            for query_i in range(n_queries_per_task + n_queries_per_task_base_dsl):
                ...
                # Keep reducing prompt size if the context is too large.
                # TODO (SA): Current query / caching scheme does not return
                # the error so this does not occur.
