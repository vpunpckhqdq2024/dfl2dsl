...
from src.python_utils import PythonExecutor
...
from dreamcoder.type import Type, t0, t1, t2, arrow, tint, tbool, baseType, tstr, tlist
...
class LAPSGrammar(Grammar):
    ...
    def checkpoint(self, experiment_state, checkpoint_directory):
        ...
        # Get declarations for the LLM-defined primitives.
        name_to_def = {}
        for p in self.primitives:
            if p.isInvented:
                continue
            name_to_def[p.name] = p.function_comment

        with open(self.get_checkpoint_filepath(checkpoint_directory), "w") as f:
            json.dump(self.json(definitions=name_to_def), f, indent=4)
    ...
    def load_model_from_checkpoint(self, experiment_state, checkpoint_directory):
        ...
        grammar = self.from_json(json_grammar, experiment_state)
        experiment_state.models[model_loaders.GRAMMAR] = grammar
        return grammar

    @staticmethod
    def from_json(json_grammar, experiment_state):
        """Loads the grammar from a checkpoint."""
        ...
        def join_prim_strings(p_strs: List[str]):
        ...
        # Declare the base primitives.
        # TODO: Handle better than string evals! Maybe pickle functions.
        prim_types = experiment_state.metadata["existing_types"]
        redefined_primitives = json_grammar.get("definitions", {})
        primitives_str = join_prim_strings(redefined_primitives.values())

        # Create an environment w/ the defined types + declare the primitives.
        str_to_type = {
        ...
        executor = PythonExecutor(globs)
        executor.run(f"_={primitives_str}")
